...
public interface MemoRepository extends JpaRepository<Memo, Long> {
    ...
    //List<Memo> 리턴타입 -> 리스트 타입의 객체는 memo
    ...
    // @Query는 순수한 sql 쿼리문으로 작성한다. 단, 테이블명이 아니라 엔티티 명으로 사용함.
    @Query("SELECT m FROM Memo m ORDER BY  m.mno DESC")
    List<Memo> getListDesc(); // 내가 만든 메서드 명
    ...
    // 쿼리문 두개 쓸때는 value="query"
    @Query(value="SELECT m FROM Memo m WHERE m.mno > :mno"
            ,countQuery = "SELECT count(m) FROM Memo m WHERE m.mno > :mno")
    Page<Memo> getListWithQuery(Long mno, Pageable pageable);

    
    //DB에 존재하지 않는 값을 처리 해보기 : 예를 들어 날짜
    @Query(value = "SELECT m.mno, m.memoText, CURRENT_DATE FROM Memo m WHERE m.mno > :mno",
    countQuery = "SELECT count(m) FROM Memo m WHERE m.mno > :mno") // countQuery 페이징 처리용
