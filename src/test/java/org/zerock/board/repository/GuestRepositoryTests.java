...
public class GuestRepositoryTests {
    ...
    public void insertDummies(){ // 테이블에 더미데이터 300개 추가
        IntStream.rangeClosed(1, 300).forEach(i -> {
            Guestbook guestbook = Guestbook.builder()
                    .title("제목..." + i)
                    .content("내용..." + i)
                    .writer("user" + (i%10)) //user 0 ~ user 9번
                    .build(); // 빌더 패턴은 엔티티 @Builder 필수
            System.out.println(guestbookRepository.save(guestbook));
            // jpa save메서드를 실행처리와 동시 출력
        });
    }
    ...
    public void testQuery1(){ // 단일 조건으로 쿼리 생성+페이징+정렬
        ...
        String keyword = "9"; // 프론트페이지에서 1번을 찾겠다는 의미

        BooleanBuilder builder = new BooleanBuilder(); // 다중 조건 처리용 객체

        BooleanExpression expressionTitle = qGuestbook.title.contains(keyword); // expression (표현) title = 1 표현식이 생성됨
        BooleanExpression expressionContent = qGuestbook.content.contains(keyword); // expression (표현) content = 1 표현식이 생성됨
        BooleanExpression exAll = expressionTitle.or(expressionContent); // 표현식 두개를 하나로 합침

        builder.and(exAll); // 다중 조건처리용 객체에 표현식을 밀어 넣음
        builder.and(qGuestbook.gno.gt(0L)); // where문 추가 (gno > 0)

        Page<Guestbook> result = guestbookRepository.findAll(builder, pageable);
        // 페이지 타입의 객체가 나온다(.findAll은 검색된 모든 객체가 나온다.)
